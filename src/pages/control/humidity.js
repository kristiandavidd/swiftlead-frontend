...
export default function HumidityDetail() {
    ...
    const [dailyData, setDailyData] = useState({ kelembapan: [], labels: [] });
    const [monthlyData, setMonthlyData] = useState({ kelembapan: [], labels: [] });
    ...
    const fetchDailyData = async () => {
        ...
        try {
            ...
            if (Array.isArray(res.data)) {
                ...
                const kelembapan = res.data.map((d) => Number(d.avgKelembapan)); // Convert to numbers
                setDailyData({ kelembapan, labels });
            } else {
    ...
    const fetchMonthlyData = async () => {
        ...
        try {
            ...
            if (Array.isArray(res.data)) {
                ...
                const kelembapan = res.data.map((d) => Number(d.avgKelembapan)); // Convert to numbers
                setMonthlyData({ kelembapan, labels });
            } else {
                console.error('Unexpected data format:', res.data);
                setMonthlyData({ kelembapan: [], labels: [] });
            }
    ...
    const chartData = {
        ...
        datasets: [
            {
                label: 'Humidity (%)',
                data: timeRange === 'daily' ? dailyData.kelembapan : monthlyData.kelembapan,
    ...
    const downloadPDF = () => {
        ...
        html2canvas(pdfRef.current).then((canvas) => {
            ...
            doc.save(timeRange === 'daily' ? 'Daily Humidity.pdf' : 'Monthly Humidity.pdf');
    ...
    return (
        <UserLayout head="Humidity Details">
            <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                    <p className="text-lg font-semibold">Humidity Dashboard</p>
                    <p className="text-sm">Real-time and historical humidity data monitoring.</p>
                </div>
            </div>

            <div className="mb-6">
                <div className="p-4 bg-white rounded-lg shadow">
                    <p className="text-sm text-gray-500">Real-Time Humidity</p>
                    <h2 className="text-2xl font-bold">{sensorData.kelembapan.toFixed(2)}%</h2>
                </div>
            </div>
            ...
            <div className="mb-6 print-content" ref={pdfRef}>
                <Tabs defaultValue="daily" onValueChange={(value) => setTimeRange(value)} className='flex flex-col gap-4'>
                    <div className='flex items-center justify-between'>
                        <div>
                            <p className="text-lg font-semibold">Humidity Trends</p>
                            <p className="text-sm">View daily and monthly humidity trends.</p>
                        </div>
                    ...
                    <div className='flex w-full p-4 bg-white rounded-lg'>
                        ...
                        <ScrollArea className="w-1/4 px-4 text-sm no-print h-72">
                            <table className="w-full border border-collapse border-gray-300 table-auto">
                                ...
                                <tbody>
                                    {(timeRange === 'daily' ? dailyData : monthlyData).kelembapan.map((item, index) => (
                                        <tr key={index}>
                                            <td className="px-4 py-2 border border-gray-300">{(timeRange === 'daily' ? dailyData : monthlyData).labels[index]}</td>
                                            <td className="px-4 py-2 border border-gray-300">{item.toFixed(2)}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
